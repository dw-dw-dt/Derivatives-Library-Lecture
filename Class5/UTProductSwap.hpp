...
//
class UTProductLinearBase : public UTProductBase
{
public:
	...
	const std::vector<std::shared_ptr<const UTProductBase> >& underlyings() const { return myUnderlyings; }
	const std::shared_ptr<const UTProductBase>& underlying(unsigned long lProduct) const { return myUnderlyings[lProduct]; }
	unsigned long size() const { return myUnderlyings.size(); }
...
public:
	...
	// Functions.
	virtual const std::string &classTag() const { return ourClassTag; }
