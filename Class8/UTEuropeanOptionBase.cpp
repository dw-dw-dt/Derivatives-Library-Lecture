...
UTEuropeanOption::intrinsicValue(double forward, double strike, UT_CallPut callPut)
{
	...
	switch (callPut)
	{
	case UT_CallPut::UT_CALL:     intrinsic = UTMathFunctions::max(forward - strike, 0.0); break;
	case UT_CallPut::UT_PUT:      intrinsic = UTMathFunctions::max(strike - forward, 0.0); break;
	case UT_CallPut::UT_STRADDLE: intrinsic = fabs(strike - forward); break;
